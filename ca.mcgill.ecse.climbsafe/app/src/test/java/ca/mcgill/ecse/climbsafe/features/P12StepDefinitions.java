...
public class P12StepDefinitions {
  ...
  public void the_following_guides_exist_in_the_system_p12( io.cucumber.datatable.DataTable dataTable ) {
    ...
    for( Map<String, String> g : guideList ) {
    	climbSafe.addGuide(g.get("email"), g.get("password"), 
    			g.get("name"), g.get("emergencyContact"));
    }
  }
  ...
  public void the_following_members_exist_in_the_system_p12( io.cucumber.datatable.DataTable dataTable ) {
    ...
    for( Map<String, String> m : memberList ) {
    	climbSafe.addMember(m.get("email"), m.get("password"), 
    			m.get("name"), m.get("emergencyContact"), 
    			Integer.parseInt(m.get("nrWeeks")), 
  ...
  /**
   * Method used to check that the state of the model is exactly what is expected from the 
  ...
  /**
   * Method used to delete the current climbsafe system instance before the next test.
   * This allows to reset and clean the climbsafe system instance at each test
   */
  @After
