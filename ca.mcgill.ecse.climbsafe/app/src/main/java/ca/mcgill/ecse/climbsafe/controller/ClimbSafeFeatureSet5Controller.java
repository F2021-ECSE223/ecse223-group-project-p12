 ...
 * bundle. Namely, its purpose is to help with the creation and updating of equipment bundles.
 *
 * @author Habib Jarweh
 */
...
public class ClimbSafeFeatureSet5Controller {
   ...
   * @param equipmentNames List of the names of the equipments we want to add to the the bundle
   *        (must be at least two names in the list of equipment names, can't have same name more
   *        than once, and names have to be valid equipment items)
   * @param equipmentQuantities List of the quantities of each equipment we want in our bundle (
   *        quantity must be > 0)
   * @throws InvalidInputException if discount is less than 0 or more than 100 if list size is less
   *         than 2 if an equipment name is listed more than once if equipment name listed is not a
   *         valid equipment name if we don't have at least a quantity of 1 of every equipment name
   *         listed
   */
  ...
  public static void updateEquipmentBundle(String oldName, String newName, int newDiscount,
    ...
    for (int i = 0; i < newEquipmentNames.size(); i++) {
      BundleItem bundleItem = new BundleItem(newEquipmentQuantities.get(i), climbSafe,
  ...
  private static void namesAreValid(List<String> equipmentNames) throws InvalidInputException {
    ...
    // equipment quantity constraints, first if statement makes sure size of equipmentNames is at
    // least 2
    ...
    for (int i = 0; i < equipmentNames.size(); i++) {
      for (int j = 0; j < climbSafe.getEquipment().size(); j++) {
        if (equipmentNames.get(i).equals(climbSafe.getEquipment().get(j).getName()))
          found = true;
      }
      // if equipment name doesn't match any name from equipments list then
      if (found == false)
        throw new InvalidInputException("Equipment " + equipmentNames.get(i) + " does not exist");
   ...
   * @author Habib Jarweh
   */
  private static void quantitiesAreValid(List<Integer> equipmentQuantities)
      throws InvalidInputException {
    // if statement (in the for loop) makes sure there is at least one of every equipment item
    // selected
   ...
   * Method for input validation. Makes sure all constraints are respected:
   * 
   * @param equipments |List<Equipment>| the list of equipment in climbSafe is going to be inputed
   *        here.
   * @param name |String| name is going to be inputed in the parameter name, or newName in the case
   *        of updateEquipmentBundle this method checks that there isn't already an equipment item
   *        with the name name
   * @throws InvalidInputException there already exists a bookable item or an equipment in this case
   *         with the name name
   * @author Habib Jarweh
   */
  private static void bookableItemIsValid1(List<Equipment> equipments, String name)
      throws InvalidInputException {
   ...
   * Method for input validation. Makes sure all constraints are respected:
   * 
   * @param bundles |List<EquipmentBundle>| the list of equipment bundles in climbSafe is going to
   *        be inputed here.
   * @param name |String| name is going to be inputed in the parameter name, or newName in the case
   *        of updateEquipmentBundle this method checks that there isn't already a bundle with the
   *        name name
   * @throws InvalidInputException there already exists a bookable item or a bundle in this case
   *         with the name name
   * @author Habib Jarweh
   */
  private static void bookableItemIsValid2(List<EquipmentBundle> bundles, String name)
      throws InvalidInputException {
   ...
   * Method for input validation. Makes sure all constraints are respected:
   * 
   * @param equipmentbundle |EquipmentBundle| bundle that we want to update, which we get using
   *        oldName.
   * @param name |String| oldName is going to be inputed in the parameter name this method finds the
   *        old equipment bundle from oldName
   * @throws InvalidInputException bundle with the name oldName does not exist
   * @author Habib Jarweh
   */
  private static void oldEquipmentBundleIsValid(EquipmentBundle equipmentbundle, String name)
      throws InvalidInputException {
    // finding old equipment bundle from oldName
    if (equipmentbundle == null)
      throw new InvalidInputException("Equipment bundle " + name + " does not exist");
  }
